...
TcpServer/UdpServer 这部分完成之后，后续主要集中在两个方向：

1. **共享栈（Shared Stack）**：优化栈内存占用
2. **协程池（Fiber Pool）**：优化 Fiber 对象和独立栈的复用效率

这两个优化都和 `Fiber / Scheduler / IOManager / Hook` 深度耦合，所以后面的工作重点不再是业务接口，而是协程运行时本身。
...
- 每个 Fiber 都有自己独立的栈；
- 栈大小通常是 128KB；
- 如果并发协程数量很多，内存占用会快速上升。

举例：

```text
10000 个协程 × 128KB ≈ 1.25GB 栈内存
```

这类内存开销在高并发短任务场景下非常浪费，因为：
...
- 每个线程维护自己的共享栈；
- Fiber 第一次在哪个线程初始化上下文，就绑定到哪个线程；
- 后续挂起/恢复都只能回到这个线程；
- 挂起时保存栈内容，恢复时仍恢复到原线程的共享栈。
...
这意味着一个 Fiber 的执行现场，本质上是：

```text
ucontext + 当前栈 = 一套完整可恢复的执行上下文
```
...
2. 挂起时把栈内容保存到 Save Buffer；
3. 恢复到 B 线程时，不再用 A 的共享栈；
4. 改用 B 线程的一块共享栈，把 Save Buffer 拷回去，再继续执行。

问题在于：

- `ucontext` 恢复执行时依赖的不只是 `uc_stack` 这个描述字段；
- 还依赖寄存器现场里的真实栈指针；
- 单独改 `uc_stack.ss_sp`，并不能安全地把一个已经建立好的上下文“换到另一块新栈上继续跑”。

#### 3.3 为什么说要“重写协程类的上下文切换逻辑”

如果真的要支持跨线程共享栈，当前这套基于 `ucontext` 的模型就不够了。

原因是：

- 现在的 `resume()/yield()` 逻辑默认上下文和栈是强绑定的；
- 而跨线程共享栈要求“同一个执行现场，恢复时可切到另一块物理栈”；
- 这不是简单改几个字段能解决的，而是要重构 `Fiber` 的上下文保存/恢复方式。

更准确地说，需要重写的包括：

1. **上下文初始化方式**
   - 不能再把 `makecontext()` 结果永久绑定到一块固定栈；

2. **栈保存/恢复方式**
   - 需要把真实运行栈内容与上下文寄存器现场拆开管理；

3. **resume/yield 切换逻辑**
   - 需要在恢复前安全切换到目标线程的新共享栈；

4. **Scheduler 协同逻辑**
   - 需要明确 Fiber 在跨线程迁移时，何时保存、何时恢复、何时切换线程。

所以我当前的结论是：

```text
当前项目结构下，真正跨线程共享栈不能靠“小修小补”完成，必须重写 Fiber 的上下文切换逻辑。
```

因此我没有强行做一个“不稳定但看起来能跑”的版本，而是选择了可上线、可验证的 **线程绑定共享栈 V1**。
...
- 构造时不一定立刻初始化共享栈上下文；
- 第一次 `resume()` 时才绑定线程并建立上下文；
- `yield()` 后由调度器侧完成共享栈保存与释放；
- 下次恢复前由调度器侧完成重新获取共享栈与恢复数据。

#### 4.2 为什么“保存栈”和“释放共享栈”要放到调度器侧

这是共享栈实现里最重要的细节之一。

如果直接在 `Fiber::yield()` 内部做：

1. 保存当前栈内容
2. 释放共享栈
3. 再 `swapcontext` 切回调度器

那会有一个严重问题：

- 当前 `yield()` 自己就还运行在这块共享栈上；
- 如果还没切走就先 release，相当于“脚还站在地板上，先把地板归还给别人”；
- 这是典型未定义行为。

所以最终实现采用“两阶段处理”：

1. Fiber 先切回调度协程；
2. 调度协程恢复后，再执行共享栈保存与 release；
3. 下次 resume 前，再由调度器做 restore。

#### 4.3 Scheduler 需要配合什么

共享栈 V1 不是 Fiber 自己单独能完成的，Scheduler 必须配合：

- 共享栈 Fiber 第一次运行后会绑定线程；
- 后续如果重新调度该 Fiber，必须携带线程 ID；
- 定时器唤醒路径和 IO 唤醒路径也都必须保留这个线程绑定信息。
...
- 实现复杂度高；
- 需要保存/恢复栈内容，有额外 memcpy 成本；
- 共享栈 Fiber 不能像独立栈 Fiber 那样自由跨线程迁移；
- 对调度器状态机和线程绑定要求更高。

一句话总结：

```text
共享栈主要优化内存，不是白拿性能；内存收益大，但运行时管理更复杂。
```
...
共享栈解决的是“栈内存占用”问题，但还有另一个问题：

- 调度器在处理大量短生命周期任务时，会频繁创建和销毁 Fiber；
- `new Fiber + 分配栈 + makecontext + 析构` 这些操作本身就有成本；
- 对高频小任务来说，这类对象生命周期管理开销会拖慢调度器。

所以协程池的目标是：

```text
复用已经执行结束的 Fiber 对象，减少频繁创建和销毁的成本。
```
...
- 状态已经结束的 Fiber（`TERM/EXCEPT`）；
- 下次需要跑新任务时，对旧 Fiber 做 `reset(cb)`；
- 复用其已有栈和上下文对象。
...
- 超大栈默认不池化，避免少量极端任务长期占住池容量。

#### 4.2 acquire 流程

获取 Fiber 的流程：

1. 看 `fiber.pool.enabled` 是否开启；
2. 根据当前配置判断是否应走共享栈路径；
3. 如果是独立栈，则先归一化栈大小；
4. 从对应桶里取空闲 Fiber；
5. 命中则 `reset(cb)` 后返回；
...
1. 只回收 `TERM/EXCEPT` 状态的 Fiber；
2. 归还前清空 callback，避免闭包继续持有资源；
3. 独立栈按标准化栈大小回桶；
4. 共享栈回共享栈池；
5. 超出池上限时直接丢弃。
...
并把 `shrink()` 接入到了 `Scheduler::idle()` 空闲路径里，让线程空闲时自动回收长期不用的 Fiber。
...
- 如果是 `TERM/EXCEPT`，归还池中；
- 如果是 `READY`，重新调度；
- 如果是 `HOLD`，说明还没结束，不能回池。
...
- 它主要优化“创建/销毁成本”，不是主要的内存优化手段；
- 如果任务本身生命周期长，池化收益就不会特别夸张；
- 和共享栈组合后，状态管理复杂度会上升。

一句话总结：

```text
协程池主要优化性能，共享栈主要优化内存，二者关注点不同。
```
...
我专门写了 benchmark，把这 16 组全部跑了一遍，目的是：
...
| 协议 | 协程池 | 共享栈 | use_caller | 结果 | QPS | 结论 |
|---|---|---|---|---|---:|---|
| TCP | off | off | off | 成功 | 12801 | 基线 |
| TCP | off | off | on | 成功 | 15399 | TCP 最优 |
| TCP | off | on | off | 成功 | 8802 | 共享栈开销明显 |
| TCP | off | on | on | 成功 | 13161 | 共享栈下仍可接受 |
| TCP | on | off | off | 成功 | 13418 | 比基线略好 |
| TCP | on | off | on | 成功 | 14733 | 高性能组合 |
| TCP | on | on | off | 成功 | 10270 | 比纯共享栈好一些 |
| TCP | on | on | on | 成功 | 13702 | 稳定且性能中上 |
| UDP | off | off | off | 成功 | 10151 | 基线 |
| UDP | off | off | on | 成功 | 11912 | 明显提升 |
| UDP | off | on | off | 成功 | 5713 | worker-only 共享栈较慢 |
| UDP | off | on | on | 成功 | 11745 | use_caller 对冲了大量成本 |
| UDP | on | off | off | 成功 | 10311 | 比基线略好 |
| UDP | on | off | on | 成功 | 12108 | UDP 最优 |
| UDP | on | on | off | 成功 | 5364 | 当前最慢组合 |
| UDP | on | on | on | 成功 | 11446 | 稳定，性能尚可 |

### 4. 结果解读

#### 4.1 `use_caller=true` 并不慢，反而常常更快

在当前实现里：

- `use_caller=true` 往往优于 `use_caller=false`；
- 原因是 caller 线程本身也参与调度，相当于少浪费一个线程上下文。
...
所以在 UDP 这种高频短 IO 场景中，性能下降更明显。

#### 4.3 协程池收益和 workload 强相关

协程池在当前 echo 微基准下不是“碾压式提升”，但它的价值在：
...
如果面试官问“最终推荐的模式是什么”，我会这样回答：

- **追求性能**：独立栈 + `use_caller=true`
- **追求内存**：共享栈 + `use_caller=true`
- **追求综合平衡**：看业务场景决定是否叠加协程池
...
因为共享栈和协程池解决的问题根本不同，组合后也不一定线性收益。

---

## sylar基础进行升级的过程中最适合面试回答的几个 Bug
...
1. `sleep/usleep/nanosleep` 等定时器唤醒路径；
2. `IOManager` 的 IO 事件唤醒路径。

最后发现问题不是共享栈保存/恢复本身，而是：

- 重新调度共享栈 Fiber 时，没有把绑定线程 ID 带回去；
- 调度器就把它当成普通 Fiber 随机分发到了别的线程。

#### 根因

- 独立栈 Fiber 可以跨线程恢复；
- 共享栈 V1 不行；
- 但调度路径里最初没有把这两种 Fiber 区分开来。

#### 修复

- 给共享栈 Fiber 增加 `m_boundThread`；
- 在 Hook 唤醒路径和 IO 唤醒路径里重新调度时，显式传入正确线程 ID；
- Scheduler 收到后强制放回绑定线程执行。
...
- 修复之前要先分清模型约束；
- 不是所有 Fiber 都有同样的调度自由度；
- 共享栈 Fiber 的线程约束比独立栈更严格。

### Bug 2：协程池已经做了分桶，但命中率异常低

#### 现象

- 协程池功能看起来没问题；
- 但实际统计发现命中率不高；
- 很多本来应该复用的 Fiber 还是重新创建了。

#### 定位过程

我按三段路径去排查：

1. `acquire()` 取桶时用的 key 是什么；
2. miss 时新建 Fiber 用的栈大小是什么；
3. `release()` 回桶时又是按什么规则放回去。

最后发现：

- 取桶时用的是“标准化后的栈大小”；
- 新建时有时还在用原始请求大小；
- 归还时又按 Fiber 自己记录的实际大小回桶；
- 三处规则不一致。

#### 根因

像 `200KB`、`192KB` 这种非标准大小：

- acquire 去 `256KB` 桶里找；
- 但 miss 后创建出来的 Fiber 可能记录的是原始大小；
- release 又回到原始大小对应的桶；
- 最终导致“取桶”和“回桶”对不上。
...
- 统一池 key 规则；
- 决定池化时，miss 路径也按归一化后的大小创建 Fiber；
- release 时也按同一规则回桶；
- 再补了专门的非标准栈大小测试。
...
- 不是所有性能问题都会直接崩；
- 很多时候程序“功能是对的”，但设计细节会让优化失效；
- 性能问题也要像功能 bug 一样按链路排查。

### Bug 3：`UDP + use_caller` 下的间歇性调度竞态

#### 现象

- 在 16 组模式压测中，`UDP + use_caller` 早期出现间歇性异常；
- 不是每次都崩，单独跑有时还正常；
- 连续压测时更容易出现状态断言问题。
...
1. 先排除 benchmark 本身误用 hook 的问题；
2. 再盯 `recvfrom -> do_io -> addEvent` 路径；
3. 最后回到调度器，看 resume 返回后的状态收尾逻辑。

#### 根因

真正的问题在 `Scheduler::run()` 的收尾逻辑：

- 某个 Fiber 在一条线程上 `YieldToHold()`；
- 另一个线程很快又把它重新调度为 `EXEC`；
- 但原线程 resume 返回后的收尾逻辑，又把它强行写回 `HOLD`；
- 于是后面再进入 hook IO 时，状态机就乱了。

#### 修复

- 修改调度器收尾逻辑；
- 如果 Fiber 状态已经是 `HOLD` 或 `EXEC`，就不再强行覆盖；
- 同时在 `IOManager::addEvent()` 增加防御性保护，遇到非法状态时回滚 epoll 注册，而不是直接断言崩溃。

#### 面试回答亮点

这个 bug 非常适合在面试里讲，因为它能体现：

- 你会用压测逼出竞态；
- 你能区分“测试代码问题”和“框架本身问题”；
- 最终定位到的是调度状态机竞争，而不是只在表面修一个断言。

### Bug 4：共享栈设计里“保存栈 / 释放共享栈”的时机错误

#### 现象

这个 bug 更偏设计级，不一定等到线上崩溃才发现。
...
因为 `yield()` 自己还在当前共享栈上执行。

如果还没 `swapcontext` 切走，就先 release 共享栈，相当于：

```text
当前函数还站在这块栈上，却先把这块栈归还给别人复用
```
...
- 不是所有 bug 都要靠日志和 core dump 才发现；
- 有些风险可以靠执行模型推演提前规避；
- 这类“在设计阶段就识别出潜在未定义行为”的能力，在系统开发里很重要。

### Bug 5：Hook 超时在多线程 `IOManager` 下偶发失效
...
- 单线程测试基本都过；
- `test_hook`、`test_http_server` 这种已有回归都没报错；
- 但在多线程 `IOManager` 下，并发做一批 socket 读超时时，会偶发返回 `EAGAIN`，而不是预期的 `ETIMEDOUT`。

它最麻烦的地方在于：

- 功能不是完全坏掉；
- 不是 100% 复现；
- 只在“多线程调度 + 超时唤醒”这个组合条件下才容易暴露。

#### 定位过程

这个 bug 的排查过程其实很适合面试里讲，因为一开始线索还是“错的”。

最初是外部分析先怀疑了两个方向：

1. `IOManager::addEvent()` 是否会因为事件标志没有及时清掉而重复注册；
2. `Fiber::resume()` 是否因为状态切换不是原子操作而被双重恢复。

我先做的不是急着改代码，而是逐层把这两个怀疑证伪：

- `addEvent()` 和 `triggerEvent()` 之间其实有同一个 `fd_ctx->mutex` 串行保护；
- `resume()` 本身虽然不是原子状态机，但主路径真正的问题并不在这里。

证伪完之后，我继续盯着真正的超时链路看：

```text
do_io()
  -> addConditionTimer(timeout, ...)
  -> addEvent(fd, event)
  -> Fiber::YieldToHold()
  -> timer / IO 就绪后恢复 fiber
```

然后发现了两个关键问题：

1. **第一次问题**：超时 timer 回调里，`cancelEvent()` 已经会唤醒 fiber，但代码又额外 `schedule(ptr)` 了一次，导致同一个 fiber 可能重复入队；
2. **更深的问题**：超时结果是通过 `errno = ETIMEDOUT` 传递的，而 `errno` 是线程局部变量。

接着我专门写了一个仓库外的复现实验：

- `IOManager(4, false)`
- 并发调度 64 个 `socketpair` 读超时任务
- 每个任务都设置 `SO_RCVTIMEO`
...
也就是说，这不是理论风险，而是真实可复现的竞态 bug。

#### 会导致的后果

这个 bug 如果不修，后果不是“立刻崩溃”，而是更难排查的那种业务级错误：

- 本来应该按“超时失败”处理的请求，被误判成“暂时没数据”；
- 上层逻辑可能继续重试，导致超时边界不准确；
- 压力一大时会出现非常难解释的偶发网络行为；
- 单线程环境正常、多线程环境异常，最容易让人误以为是业务代码问题。

换句话说，它会把一个本该**确定失败**的超时，变成一个**不稳定的伪重试信号**。
...
- timer 回调里调用 `cancelEvent(fd, event)`；
- `cancelEvent()` 内部已经会 `triggerEvent()`，把等待中的 fiber 重新调度；
- 原代码又额外 `schedule(ptr)`；
- 同一个 fiber 出现重复入队风险。

第二层才是最难处理、也最值得讲的：

- `do_io()` 原来用 `errno == ETIMEDOUT` 判断“本次唤醒是不是超时”；
- 但 timer 回调执行在线程 A；
- 被唤醒后的 fiber 可能在调度线程池里的线程 B 上恢复；
- `errno` 是 **thread local**；
- 于是线程 A 写进去的 `ETIMEDOUT`，在线程 B 上根本看不到。

这就是为什么它在：

- 单线程 `IOManager` 下通常没事；
- 多线程 `IOManager` 下才会偶发失效。
...
- 去掉 timer 超时回调里的额外 `schedule(ptr)`；
- 保留 `cancelEvent()` 作为唯一唤醒路径。

**第二步：把超时结果从线程局部 `errno` 改成共享状态**

- 引入 `timer_info` 共享对象；
- 用原子字段记录 `cancelled = ETIMEDOUT`；
- fiber 恢复后不再直接看当前线程的旧 `errno`，而是先看共享超时状态，再决定是否写回 `errno` 返回。

**第三步：保证“先写超时标记，再唤醒 fiber”**

这是修复里最关键的一点。

我把 `IOManager::cancelEvent()` 扩展成支持一个可选的 `on_cancel` 回调：

- 先在 `cancelEvent()` 内部拿着 `fd_ctx->mutex`
- 先执行 `on_cancel()`，把共享超时状态写好
- 再 `triggerEvent()` 唤醒 fiber

这样无论 fiber 最终在哪个线程恢复，都能稳定看到已经写好的超时结果。

**额外优化**

为了不影响快路径性能，后面又做了一步优化：

- `do_io()` 不再每次调用都预先创建超时状态和 timer；
- 只有真正遇到 `EAGAIN/EINTR`、并且配置了超时时，才懒创建这些对象。

也就是说：

- 成功快路径几乎不受影响；
- 增加的同步开销只落在“本来就要挂起等待”的慢路径上。

#### 为什么之前测试没测出来

这也是这次 bug 很值得讲的一点。

旧测试之所以没测出来，不是因为逻辑完全正确，而是因为测试环境刚好把 bug 遮住了：

- 原有 `test_hook` 用的是单线程 `IOManager`
- 单线程下 timer 回调线程和 fiber 恢复线程通常就是同一条线程
- 于是 `errno` 的线程局部问题不会暴露
...
1. **单线程回归**
   - 验证超时唤醒不会重复恢复同一个 fiber；

2. **多线程回归**
   - 新增 `test_hook_timeout_mt`
   - 在 4 个 worker 下并发跑 64 个 socket 超时任务
   - 断言全部返回 `ETIMEDOUT`
   - 防止将来再退化回 `EAGAIN`

#### 面试回答亮点

这个 bug 非常适合拿来回答“最难处理的 bug 是什么”，因为它能同时体现：

- 你不会被第一眼看上去很像的怀疑方向带偏；
- 你能先证伪错误猜想，再聚焦真正根因；
- 你理解线程局部状态、协程恢复线程、事件取消、调度唤醒之间的关系；
- 你不是只修表面症状，而是把“重复调度”和“跨线程超时传播”两层问题一起收口；
- 你修完之后还补了多线程定向测试，并顺手把快路径性能开销优化掉。

---

## 阶段总结

TcpServer/UdpServer 完成之后，我后续的核心工作主要就是两块：

1. **共享栈**：解决大量协程下的栈内存占用问题；
2. **协程池**：解决高频短任务下 Fiber 创建/销毁成本问题。

这两项优化最后都不是只改一个类，而是联动修改了：
