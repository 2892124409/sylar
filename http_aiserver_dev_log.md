...
#### 第二阶段补充：HttpServer 健壮性增强

本阶段给 `HttpServer + HttpRequestParser` 增加了“请求大小限制”的最小实现。

**新增能力：**

1. 请求头大小限制
   - `HttpRequestParser::SetMaxHeaderSize()`
   - 默认 8KB

2. 请求体大小限制
   - `HttpRequestParser::SetMaxBodySize()`
   - 默认 10MB

3. 解析错误分类
   - 普通格式错误：按 `400 Bad Request` 处理
   - 请求过大：按 `413 Payload Too Large` 处理

**当前实现方案：**

- header 未收完整但缓冲区已超过上限 -> 直接报“header too large”
- `Content-Length` 超过 body 上限 -> 直接报“body too large”
- `HttpServer::handleClient()` 根据解析错误类型返回 400 或 413
...
1. 目前限制项是解析器静态全局值，后续可改成配置项或服务实例级配置
2. 当前只做了最小错误分类，后续可细化更多错误码和错误体格式
3. 目前还没实现更丰富的超时/限流/观测能力
...
1. `test_http_parser`
   - 新增 header/body 过大场景测试

2. `test_http_server`
   - 新增服务端返回 `413 Payload Too Large` 的集成验证

#### 第二阶段补充：路由层扩展点说明

本阶段没有继续改造 `ServletDispatch` 的核心实现，但明确了当前路由层的边界和后续扩展方向。
...
1. 当前路由能力已经足够支撑“通用 HTTP 框架 + SSE demo”
2. 现阶段主要瓶颈不在路由，而在 Session 生命周期和流式输出链路
3. 保持 `vector` + 线性查找，更利于当前学习和调试

**后续可扩展方向：**

1. 更高效的路由结构
   - 例如 `unordered_map` 精确匹配
   - 或前缀树 / Trie 路由
...
3. 前后置处理钩子
   - 请求进入前统一鉴权/日志
   - 响应返回前统一补 header / 观测信息

4. 中间件链模型
   - 让路由处理从“单点 handle”演化成“责任链式处理”

**当前实现 / 后续优化的取舍结论：**

- **当前实现**：保留最小可用、易理解的路由分发结构
- **后续优化**：当业务类型和路由规模明显扩大后，再演进为更复杂的路由/中间件体系
...
#### 第二阶段补充：Session 生命周期增强

本阶段在第一版内存 Session 之上，补充了“周期性过期清理”能力。

**新增能力：**

1. `SessionManager::startSweepTimer()`
   - 可把 `SessionManager` 挂到 `IOManager/TimerManager` 上
   - 以固定周期自动执行 `sweepExpired()`

2. `SessionManager::stopSweepTimer()`
   - 可取消周期性清理任务

3. `SessionManager::hasSweepTimer()`
   - 可判断当前是否已启用自动清理

**当前实现方案：**

- 请求访问路径上：`get()` 命中过期会话时立即删除
- 定时器路径上：周期性批量执行 `sweepExpired()`

这是一种“请求时顺带清理 + 后台定时清理”的组合方案。

**为什么当前这样实现：**

1. 实现简单，易于验证
2. 不依赖外部存储
3. 即使某个过期 Session 长时间未被访问，也能被后台定时器清掉

**当前局限 / 后续可优化：**

1. 目前仍是纯内存 Session，不支持跨进程共享
2. 定时清理周期是固定值，后续可改成配置项
3. 未来可考虑加入 Session 数量上限、惰性淘汰或 LRU 策略

#### 第二阶段测试补充

新增 `test_session_manager`，覆盖：
...
本阶段把原本“只会格式化和写出”的 `SSEWriter`，真正接入到 HTTP 服务端链路里做了一个通用 demo。

**当前实现方案：**

1. 在 `HttpResponse` 中新增“流式响应”语义
   - `setStream(true)` 表示这次响应由业务代码自行写出
   - `toHeaderString()` 用于仅序列化响应头，不自动补 `Content-Length`

2. 在 `HttpServer::handleClient()` 中识别流式响应
   - 普通响应：仍走 `session->sendResponse(response)`
   - 流式响应：跳过默认发送逻辑，由业务代码手动写 header 和 body

3. 新增 `/events` SSE demo（测试中验证）
...
1. 不需要立即引入 chunked response 实现
2. 保持现有普通 HTTP 响应链路不变
3. 用最小改动先验证框架确实能承载流式输出

**当前局限 / 后续可优化：**

1. 当前 SSE demo 采用“流结束后关闭连接”的方式，没有做连接复用
2. 还没有抽象成更高级的 SSE Servlet/BaseServlet
3. 后续可考虑支持 chunked response，使流式输出和 keep-alive 更自然结合
