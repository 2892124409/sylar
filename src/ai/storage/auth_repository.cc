...
bool AuthRepository::MergeGuestDataToPrincipal(const std::string& guest_sid,
                                               const std::string& principal_sid,
                                               std::string& error)
{
    if (guest_sid.empty() || principal_sid.empty() || guest_sid == principal_sid)
    {
        return true;
    }
    ...
    const std::string safe_guest_sid = Escape(conn.get(), guest_sid);
    const std::string safe_principal_sid = Escape(conn.get(), principal_sid);
    ...
    do
    {
        std::ostringstream upsert_conversations;
        upsert_conversations
            << "INSERT INTO conversations (sid, conversation_id, created_at_ms, updated_at_ms, summary_text, summary_updated_at_ms) "
               "SELECT '"
            << safe_principal_sid
            << "', c.conversation_id, c.created_at_ms, c.updated_at_ms, c.summary_text, c.summary_updated_at_ms "
               "FROM conversations c WHERE c.sid='"
            << safe_guest_sid
            << "' ON DUPLICATE KEY UPDATE "
               "updated_at_ms=GREATEST(conversations.updated_at_ms, VALUES(updated_at_ms)), "
               "summary_text=IF(CHAR_LENGTH(VALUES(summary_text)) > CHAR_LENGTH(conversations.summary_text), "
               "VALUES(summary_text), conversations.summary_text), "
               "summary_updated_at_ms=GREATEST(conversations.summary_updated_at_ms, VALUES(summary_updated_at_ms))";
        if (!ExecuteSql(conn.get(), upsert_conversations.str(), error))
        {
            ok = false;
            break;
        }

        std::ostringstream update_messages;
        update_messages << "UPDATE chat_messages SET sid='"
                        << safe_principal_sid
                        << "' WHERE sid='"
                        << safe_guest_sid
                        << "'";
        if (!ExecuteSql(conn.get(), update_messages.str(), error))
        {
            ok = false;
            break;
        }

        std::ostringstream delete_guest_conv;
        delete_guest_conv << "DELETE FROM conversations WHERE sid='" << safe_guest_sid << "'";
        if (!ExecuteSql(conn.get(), delete_guest_conv.str(), error))
        {
            ok = false;
            break;
        }
