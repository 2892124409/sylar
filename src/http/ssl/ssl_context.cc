...
bool SslContext::initialize()
{
    ...
    {
        // 输出关键错误日志，便于定位初始化失败点。
        BASE_LOG_ERROR(g_logger) << "SSL_CTX_new failed";
        // 初始化失败，终止流程。
        return false;
    }
    ...
    if (m_mode == SslMode::SERVER)
    {
        ...
        {
            // 配置不完整时记录日志。
            BASE_LOG_ERROR(g_logger) << "SSL server config incomplete";
            // 终止初始化。
            return false;
        }
        ...
        if (SSL_CTX_use_certificate_file(m_ctx, m_config.getCertificateFile().c_str(), SSL_FILETYPE_PEM) != 1)
        {
            ...
            BASE_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_file failed: " << m_config.getCertificateFile();
            // 终止初始化。
            return false;
        }
        ...
        {
            // 私钥加载失败时输出具体路径。
            BASE_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file failed: " << m_config.getPrivateKeyFile();
            // 终止初始化。
            return false;
        }
        ...
        {
            // 不匹配时无法进行安全握手。
            BASE_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key failed";
            // 终止初始化。
            return false;
        }
    }
    ...
    if (!m_config.getCaFile().empty())
    {
        ...
        {
            // 加载失败时输出路径信息。
            BASE_LOG_ERROR(g_logger) << "SSL_CTX_load_verify_locations failed: " << m_config.getCaFile();
            // 终止初始化。
            return false;
        }
    }
