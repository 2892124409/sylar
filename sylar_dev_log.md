...
### 10. V1 生产化第一轮硬化（2026-03-06）

在 V1 实验版跑通之后，已经完成了第一轮偏生产化的硬化，主要目标不是继续扩功能，而是降低误用风险、提升可观测性、收敛配置边界。

已完成的硬化点：

1. **模式保护**
   - 共享栈不再在所有调度场景下盲目启用；
   - 当前只在“已验证的调度模式”中启用 V1；
   - 对未验证模式（例如缺少可用 Scheduler 上下文的场景），会自动回退到独立栈，并记录 fallback 统计。

2. **共享栈大小一致性保护**
   - `ThreadLocalSharedStack` 不再把共享栈大小固定写死在实现里；
   - 首次启用时按配置写入全局共享栈大小；
   - 后续如果尝试使用不同共享栈大小，会直接失败，防止同一进程中出现不一致的共享栈尺寸。

3. **运行统计与可观测性**
   - 增加共享栈运行统计接口：
     - `Fiber::GetSharedStackStats()`
     - `Fiber::ResetSharedStackStats()`
   - 当前已统计：
     - 线程绑定次数；
     - `prepare/finalize` 次数；
     - `save/restore` 次数；
...
4. **回退行为测试**
   - 增加 `test_shared_stack_guard`；
   - 验证：在不满足 V1 启用条件的场景下，打开 `fiber.use_shared_stack` 不会误进共享栈路径，而是自动退回独立栈，并且统计可观测。
...
即：当前两种主要调度模式下，V1 线程绑定共享栈都已经打通并通过专项测试。

当前仍需保留的边界说明主要有：

1. 当前完成的是 **V1 线程绑定共享栈**，不是最初设想的 V2 真正跨线程共享栈；
2. 共享栈统计已经可读，但还没有接入统一监控出口；
3. 如需进一步上线使用，仍建议结合真实业务做更长时间压测与数据采样。


### 12. 下一步建议

后续如果继续朝生产级推进，优先级建议如下：

1. **异常路径与 reset 路径验证**
   - 覆盖抛异常结束、重复 reset、共享栈 Fiber 生命周期回收等边界场景；
2. **更长时间压力测试**
   - 高并发反复切换、大量 Fiber 循环创建销毁；
3. **监控与日志收口**
   - 把共享栈统计纳入统一状态输出；
4. **配置与文档收口**
   - 明确 V1 的启用条件、限制条件和回退行为。

补充：经过后续实现与验证，`use_caller=false` 的 worker-only 模式也已打通，因此这里的“启用条件”不再限制为 `use_caller=true`，而是限制为：

- 必须存在有效的 Scheduler 调度上下文；
- 不在 Scheduler 上下文中的 Fiber 仍会自动回退到独立栈。
