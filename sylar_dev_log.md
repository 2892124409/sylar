...
#### 遇到的问题
##### Q: 既然代码结构和 Mutex 没区别，为什么说它们有本质不同？
**A**: 区别在于“拿不到锁时的行为”。
- **Mutex**: 线程去“睡觉”（挂起），让出 CPU 给别人。代价是昂贵的上下文切换。
- **Spinlock**: 线程在门口“疯狂拧把手”（死循环），不放 CPU。代价是高昂的 CPU 占用。
...
3. **实现锁的逻辑**: 
   ...
   - **语义解释**: “我尝试去锁门，如果它告诉我门本来就是开着的，那我就抢占成功了；否则我就死等。”
