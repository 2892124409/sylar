...
void run_in_fiber()
{
    BASE_LOG_INFO(g_logger) << "【子协程】业务逻辑开始执行 (RunInFiber begin)";

    // 关键点 1：子协程主动让出执行权，切回主协程
    BASE_LOG_INFO(g_logger) << "【子协程】执行到一半，主动 Yield 让出 CPU";
    sylar::Fiber::YieldToHold();

    // 关键点 2：当主协程再次 resume 之后，会从刚才 yield 的下一行继续跑
    BASE_LOG_INFO(g_logger) << "【子协程】重新获得了 CPU，继续执行收尾工作 (RunInFiber end)";
...
void test_fiber()
{
    BASE_LOG_INFO(g_logger) << "【主协程】测试开始 (test_fiber begin)";

    {
        ...
        BASE_LOG_INFO(g_logger) << "【主协程】创建子协程对象";
        sylar::Fiber::ptr fiber(new sylar::Fiber(run_in_fiber));

        // 第一次切入：主 -> 子
        BASE_LOG_INFO(g_logger) << "【主协程】第一次 Resume 切入子协程";
        fiber->resume();

        // 当子协程在里面调了 YieldToHold 之后，代码会“瞬间移动”回到这里
        BASE_LOG_INFO(g_logger) << "【主协程】从子协程切回来了，刚才子协程在里面调了 yield";

        // 第二次切入：主 -> 子 (从上次停下的地方继续)
        BASE_LOG_INFO(g_logger) << "【主协程】第二次 Resume，让子协程把剩下的活干完";
        fiber->resume();

        BASE_LOG_INFO(g_logger) << "【主协程】子协程已经 TERM（结束）了";

        // 关键点 4：协程对象的复用 (Object Pool 思想)
        BASE_LOG_INFO(g_logger) << "【主协程】测试 reset 性能优化：重用刚才的栈空间跑新任务";
