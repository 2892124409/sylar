...
{
    BASE_LOG_INFO(g_logger) << "=== 测试1: 基本获取和归还 ===";
...
void test_scheduler_integration()
{
    ...
    // 输出每个线程的池统计
    BASE_LOG_INFO(g_logger) << "Scheduler任务完成";
}
...
void test_normalized_stack_buckets()
{
    ...
    BASE_LOG_INFO(g_logger) << "fiber1 stack_size=" << fiber1->getStackSize();
    fiber1->resume();
    ...
    BASE_LOG_INFO(g_logger) << "fiber2 stack_size=" << fiber2->getStackSize();

    if (fiber2.get() == raw1)
    {
        BASE_LOG_INFO(g_logger) << "✓ 非标准栈大小命中同一分桶并复用成功";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ 非标准栈大小未命中同一分桶";
    }
...
void test_configured_default_stack_size()
{
    ...
    auto stack_cfg = base::Config::Lookup<uint32_t>("fiber.stack_size", 128 * 1024, "fiber stack size");
    ...
    auto fiber1 = pool->acquire([]() {});
    BASE_LOG_INFO(g_logger) << "配置后的默认栈大小=" << fiber1->getStackSize();
    fiber1->resume();
    auto raw1 = fiber1.get();
    pool->release(fiber1);
    fiber1.reset();
    ...
    if (fiber2.get() == raw1)
    {
        BASE_LOG_INFO(g_logger) << "✓ 默认栈大小配置生效，且可复用";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ 默认栈大小配置未命中复用";
    }
...
void test_shared_stack_pooling_with_config()
{
    ...
    auto shared_cfg = base::Config::Lookup<bool>("fiber.use_shared_stack", false, "fiber use thread-bound shared stack");
    ...
    {
        BASE_LOG_INFO(g_logger) << "✓ 共享栈模式下协程池复用生效";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ 共享栈模式下协程池未命中复用";
    }

    if (shared_stats.save_count > 0 && shared_stats.restore_count > 0)
    {
        BASE_LOG_INFO(g_logger) << "✓ 共享栈保存/恢复路径已执行";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ 共享栈保存/恢复路径未执行";
    }

    if (steps == 4)
    {
        BASE_LOG_INFO(g_logger) << "✓ 共享栈调度链路执行完成";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ 共享栈调度链路执行步数异常, steps=" << steps;
    }
...
void test_shared_stack_pooling_worker_only()
{
    ...
    if (steps.load() == 3)
    {
        BASE_LOG_INFO(g_logger) << "✓ worker-only 共享栈调度链路执行完成";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ worker-only 共享栈调度链路执行步数异常, steps=" << steps.load();
    }

    if (yielded_fiber_thread_mismatch.load() == 0 && yielded_fiber_first_tid.load() != 0)
    {
        BASE_LOG_INFO(g_logger) << "✓ worker-only 模式保持线程绑定";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ worker-only 模式线程绑定异常";
    }

    if (stats.pool_hit > 0 && shared_stats.save_count > 0 && shared_stats.restore_count > 0)
    {
        BASE_LOG_INFO(g_logger) << "✓ worker-only 共享栈协程池复用生效";
    }
    else
    {
        BASE_LOG_INFO(g_logger) << "✗ worker-only 共享栈协程池未命中复用";
    }
...
{
    BASE_LOG_INFO(g_logger) << "========== 协程池功能测试 ==========";
