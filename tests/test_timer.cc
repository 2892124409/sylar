...
{
    BASE_LOG_INFO(g_logger) << "定时器触发，count = " << count;
    if (++count == 5)
    {
        // 执行 5 次后，将该定时器重置为 2 秒触发一次，且不循环
        BASE_LOG_INFO(g_logger) << "重置定时器，改为 2 秒触发一次且不再循环";
        s_timer->reset(2000, true);
    }
    if (count == 10)
    {
        // 虽然重置了不循环，但由于我们逻辑里 reset 是重插，这里作为演示
        BASE_LOG_INFO(g_logger) << "达到 10 次，彻底取消定时器";
        s_timer->cancel();
    }
}
...
int main()
{
    ...
    BASE_LOG_INFO(g_logger) << "添加 1 秒循环定时器";
    s_timer = iom.addTimer(1000, test_timer_callback, true);

    // 2. 添加一个一次性定时器
    iom.addTimer(3000, []()
                 { BASE_LOG_INFO(g_logger) << "【一次性任务】3 秒钟到了！"; });

    // 3. 添加一个 5 秒后的任务，用于关闭调度器
    iom.addTimer(15000, []()
                 { BASE_LOG_INFO(g_logger) << "【自毁任务】15 秒到了，准备结束程序"; });
